			...
			for ev := range sub.Events {
				...
				if ev.Kind == 0 {
					...
					if notFoundError != nil {
						...
						fmt.Printf("updating follows for %s: %s\n", person.Name, person.PubkeyHex)
					}
					...
					for _, c := range allPTags {
						...
						if notFoundFollow != nil {
							...
							db.Model(&person).Association("Follows").Append(&newUser)
						} else {
							// follow user found,
							db.Model(&person).Association("Follows").Append(&followPerson)
						}
					}
