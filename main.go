...
func doRelay(db *gorm.DB, seenMessages *generic_sync.MapOf[string, bool], ctx context.Context, url string) {
	...
	go func() {
		for cErr := range relay.ConnectionError {
			if cErr != nil {
				...
				doRelay(db, seenMessages, ctx, relay.URL)
			}
		}
	}()
...
func main() {
	...
	for _, url := range relayUrls {
		doRelay(db, seenMessages, ctx, url)
	}
